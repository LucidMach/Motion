...
class TestDirectionalRouting(unittest.TestCase):
    ...
    def test_parse_arrival_datetime(self):
        import testPTVOpenData
        # 1. Datetime object passthrough
        dt_obj = datetime(2026, 8, 20, 9, 15, 0)
        self.assertEqual(testPTVOpenData.parse_arrival_datetime(dt_obj), dt_obj)

        # 2. ISO timestamp strings
        dt_str = "2026-08-20 09:15"
        res_dt = testPTVOpenData.parse_arrival_datetime(dt_str)
        ...
        res_epoch = testPTVOpenData.parse_arrival_datetime(epoch)
        self.assertEqual(res_epoch.year, 2026)
        self.assertEqual(res_epoch.hour, 9)
        self.assertEqual(res_epoch.minute, 15)

        # 4. HH:MM string
        res_hm = testPTVOpenData.parse_arrival_datetime("14:30")
    ...
    def test_is_alert_active_at_time(self):
        ...
        self.assertTrue(testPTVOpenData.is_alert_active_at_time(periods_active, target_arrival_dt))

        # Case 2: Future alert starting at 11:00 AM (after arrival at 10:00 AM)
        periods_future = [{"start": target_ts + 3600, "end": target_ts + 7200}]
        self.assertFalse(testPTVOpenData.is_alert_active_at_time(periods_future, target_arrival_dt))
        ...
        self.assertFalse(testPTVOpenData.is_alert_active_at_time(periods_past, target_arrival_dt))

        # Case 4: Unconstrained / Ongoing alert (no end date)
        periods_ongoing = [{"start": target_ts - 3600, "end": 0}]
        self.assertTrue(testPTVOpenData.is_alert_active_at_time(periods_ongoing, target_arrival_dt))

        # Case 5: Empty active periods (default to active)
        self.assertTrue(testPTVOpenData.is_alert_active_at_time([], target_arrival_dt))
